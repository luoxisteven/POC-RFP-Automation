...
class RAG_Chain:
    ...
    def simple_query_answering_verbose(self, query):
        company_general_context, rfp_general_context = self.get_context()
        company_local_context = self.extract_company_local_context(query)
        
        rfp_local_context = self.extract_RFP_local_context(query)
        answer = self.query_answering_with_context(query, company_general_context, company_local_context, rfp_general_context, rfp_local_context)
        # answer = "<strong>RFP_Local_Context:</strong>\n\n" + rfp_local_context + "\n\n<strong>Company_Local_Context:</strong>\n\n" + company_local_context + \
        #     "\n\n<strong>RFP_General_Context:</strong>\n\n" + rfp_general_context + "\n\n<strong>Company_General_Context:</strong>\n\n" + company_general_context + \
        #     "\n\n<strong>Answer:</strong>\n\n" + answer
        answer = {"rfp_local_context": rfp_local_context, "company_local_context": company_local_context, \
            "rfp_general_context": rfp_general_context, "company_general_context": company_general_context, "answer": answer}
        return answer
    ...
    def detailed_query_answering_verbose(self, query):
        company_general_context, rfp_general_context = self.get_context()
        company_local_context = self.extract_company_local_context(query)
        
        rfp_local_context = self.extract_RFP_local_context(query)
        simple_answer = self.query_answering_with_context(query, company_general_context, company_local_context, rfp_general_context, rfp_local_context)
        jsonized_answer = load_json(self.convert_init_answer_to_json(simple_answer))
        detailed_answer = self.making_detailed_answer(query, jsonized_answer)
        # answer = "<strong>RFP_Local_Context:</strong>\n\n" + rfp_local_context + "\n\n<strong>Company_Local_Context:</strong>\n\n" + company_local_context + \
        #     "\n\n<strong>RFP_General_Context:</strong>\n\n" + rfp_general_context + "\n\n<strong>Company_General_Context:</strong>\n\n" + company_general_context + \
        #     "\n\n<strong>Answer:</strong>\n\n" + detailed_answer
        answer = {"rfp_local_context": rfp_local_context, "company_local_context": company_local_context, \
            "rfp_general_context": rfp_general_context, "company_general_context": company_general_context, "answer": detailed_answer}
        return answer
    ...
    def write_company_overview(self):
        if self.verbose:
            print("\nExtracting company overview..")
        company_general_context, rfp_general_context = self.get_context()
        company_overview_extraction_chain = create_chain(self.model, company_overview_extraction_prompt)
        company_overview_chain = create_chain(self.model, company_overview_prompt)
        context = ""
        for doc in self.company_docs:
            response = company_overview_extraction_chain.invoke({"context": [doc], "project_overview": rfp_general_context})
            print(response)
            print("---------------------")
            if "doesn't provide any relevant information" not in response:
                context = context + response + "\n"
        company_overview = company_overview_chain.invoke({"context": [Document(page_content=context)], 
                                                          "project_overview": rfp_general_context,
                                                          "company_profile":company_general_context,})
        print(company_general_context)
        print("---------------------")
        print(company_overview)
        return company_overview
    
    def rfp_time_extraction(self):
        time_extraction_chain = create_chain(self.model, time_extraction_prompt)
        time_info = ""
        for doc in self.company_docs:
            response = time_extraction_chain.invoke({"context": [doc]})
            if "doesn't provide any relevant information" not in response:
                time_info = time_info + response + "\n"
        return time_info
    
    def write_general_methodology_and_implementation(self):
        if self.verbose:
            print("\nWriting general methodology and implementation..")
        company_general_context, rfp_general_context = self.get_context()
        time_info = self.rfp_time_extraction()
        print("time info:" + time_info)
        general_methodology_and_implementation_chain = create_chain(self.model, general_methodology_and_implementation_prompt)
        general_methodology_and_implementation = general_methodology_and_implementation_chain.invoke({"context": [Document(page_content=rfp_general_context)], "time": time_info})
        return general_methodology_and_implementation
    
    
    def write_methodology_and_implementation(self):
        if self.verbose:
            print("\nWriting methodology and implementation..")
        company_general_context, rfp_general_context = self.get_context()

        return
    
    def write_proposal(self):
        company_overview = self.write_company_overview()
        methodology_and_implementation = self.write_methodology_and_implementation()
